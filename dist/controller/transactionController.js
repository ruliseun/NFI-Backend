...
async function Deposit(req, res, next) {
    try {
        const validateInput = utils_1.depositSchema.validate(req.body, utils_1.options);
        if (validateInput.error) {
            return res.status(400).json({ Error: validateInput.error.details[0].message });
        }
        const verified = req.user;
        const loggedInUserId = verified?.id;
        const sourceUser = await userModel_1.UserInstance.findOne({ where: { id: loggedInUserId } });
        const destinationUser = await userModel_1.UserInstance.findOne({ where: { id: req.body.userId } });
        if (!destinationUser) {
        ...
        const { accountBalance } = destinationUser;
